...
// FILTER BUTTON ROW
  ...
  //  filter on category (either use .filter or a loop)
    if (e.target.id === "music"){
      ...
      const newDataArray= data.filter((video)=>
       video.category === "javascript");
       cardsOnDom(newDataArray);
  ...
  document.querySelector('#cardContainer').addEventListener('click', (e) => {
    ...
    if (e.target.id) {
      ...
      if (e.target.id.includes('watch')) {
        ...
        //Play the video
        videoPlayer(videoId);
        //Or you can use videoPlayer(data[index].videoId)
      ...
      if (e.target.id.includes('delete')) {
        ...
        //delete the index that was found up top with specified ID
        data.splice(index, 1)
        cardsOnDom(data);
      }
    }
